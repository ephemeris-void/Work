...
asm() {
    ...
    g++ -O0 -S -masm=intel "$FILE" -o "${BASE}_O0.s"
    g++ -O3 -S -masm=intel "$FILE" -o "${BASE}_O3.s"
    diff "${BASE}_O0.s" "${BASE}_O3.s"
}

obj() {
    header "[2] OBJDUMP — machine instructions"
    g++ -O2 -g "$FILE" -o "${BASE}.out"
...
case $MODE in
    ...
    mem)   mem ;;
    gdb)   gdb_run ;;
    igdb)  g++ -O0 -g "$FILE" -o "${BASE}_gdb.out" && gdb "./${BASE}_gdb.out" ;;
    run0)  g++ -O0 "$FILE" -o "${BASE}.out" && "./${BASE}.out" ;;
    run2)  g++ -O2 "$FILE" -o "${BASE}.out" && "./${BASE}.out" ;;
    run3)  g++ -O3 "$FILE" -o "${BASE}.out" && "./${BASE}.out" ;;
    all)   asm; obj; run_perf; bench; mem; gdb_run ;;
    *)     echo "Usage: cruncpp <file.cpp> [asm|obj|perf|bench|mem|gdb|igdb|run0|run2|run3|all]" ;;
esac